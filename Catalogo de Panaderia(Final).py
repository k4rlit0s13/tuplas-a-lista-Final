...
if option==1:
    ...
    for i, val in enumerate(Productosal):
        print(f"{i}. {val} ${valoressal[i]}")

    opcion=int(input("Seleccione un producto(numero):"))
    cantidad=int(input("Ingrese la cantidad que desea comprar: "))
    total=valoressal[opcion]*cantidad

    print(f"Usted seleccionó el producto {Productosal[opcion]} con un valor de ${total}")

    dinero = int(input("Ingrese la cantidad de dinero disponible: "))
    vuelto = dinero - total
    
    if dinero >= total:
        print(f"Usted compró el producto {Productosal[opcion]} con un valor de ${total}, sus vueltos son ${vuelto}")
    else:
        print(f"Le falta dinero, necesita ${-vuelto} más para comprar este producto.")
...
if option==2:
    ...
    for i, val in enumerate(Productospastel):
        print(f"{i}. {val} ${valorespastel[i]}")

    opcion=int(input("Seleccione un producto(numero):"))
    cantidad=int(input("Ingrese la cantidad que desea comprar: "))
    total=valorespastel[opcion]*cantidad

    print(f"Usted seleccionó el producto {Productospastel[opcion]} con un valor de ${total}")

    dinero = int(input("Ingrese la cantidad de dinero disponible: "))
    vuelto = dinero - total
    
    if dinero >= total:
        print(f"Usted compró el producto {Productospastel[opcion]} con un valor de ${total}, sus vueltos son ${vuelto}")
    else:
        print(f"Le falta dinero, necesita ${-vuelto} más para comprar este producto.")
